...
from ddi_re.re_task.load_data import load_tokenize
...
class Trainer(object):
    ...
    def evaluate(self, mode):
        ...
        if mode == 'dev':
            ...
            raise Exception("Only dev re_dataset available")
    ...
    def predict(self, mode):
        ...
        output_test_file = os.path.join(self.args.model_dir, 'test_output.tsv')
        logger.info("***** Writing results *****")

        label_index = [label.strip() for label in open(self.args.label_file, 'r', encoding='utf-8')]
        with open(output_test_file, 'w', encoding='utf-8') as f:
            for label in preds:
                relation = label_index[int(label)]
                f.write(relation)
                f.write("\n")
