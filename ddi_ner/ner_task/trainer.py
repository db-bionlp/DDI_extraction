...
from ddi_ner.ner_task.load_data import load_tokenize
...
class Trainer(object):
    def __init__(self, args, train_dataset=None, dev_dataset=None, test_dataset=None):
        ...
        if args.local_rank == -1:
            ...
            # dist_init_method = 'tcp://{master_ip}:{master_port}'.format(master_ip='127.0.0.1', master_port='1001')
            # dist_world_size = 2  # total number of distributed processes.
    ...
    def evaluate(self, mode):
        ...
        # Eval!
        logger.info("\n***** Running evaluation on %s re_dataset *****", mode)
        ...
        eval_out_file = os.path.join(self.args.model_dir, 'eval_output.tsv')
    ...
    def predict(self, mode):
        ...
        # Test!
        logger.info("\n***** Running prediction on %s re_dataset *****", mode)
        logger.info("  Num examples = %d", len(dataset))
        logger.info("  Batch size = %d", self.args.eval_batch_size)
        # test_loss = 0.0
        nb_test_steps = 0
        ...
        eval_out_file = os.path.join(self.args.model_dir, 'test_output.tsv')

        with open(eval_out_file, 'w', encoding='utf-8') as f:
            for sent in final_preds:
                for word in sent:
                    if word==0:
                        f.write("B"+"\t")
                    if word==1:
